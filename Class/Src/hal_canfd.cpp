...
/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
CAN_CLASS::CAN_CLASS(FDCAN_HandleTypeDef* hfdcan)
    :hfdcan(hfdcan)
{
...
HAL_ApiState CAN_CLASS::CAN_FilterConfig(void)
{
    ...
    if (HAL_FDCAN_ConfigFilter(this->hfdcan, &stFilterConfig) != HAL_OK)
    {
...
HAL_ApiState CAN_CLASS::CAN_ActivateTxNotification(void)
{
    ...
    if (HAL_FDCAN_ActivateNotification(this->hfdcan, FDCAN_IT_TX_COMPLETE|FDCAN_IT_TX_FIFO_EMPTY, FDCAN_TX_BUFFER0) != HAL_OK)
    {
...
HAL_ApiState CAN_CLASS::CAN_ActivateRxNotification(void)
{
    ...
    if (HAL_FDCAN_ActivateNotification(this->hfdcan, FDCAN_IT_RX_FIFO0_NEW_MESSAGE, 0) != HAL_OK)
    {
...
HAL_ApiState CAN_CLASS::CAN_AddTxMessage(CanMsg_st_t *stCanTxMsg)
{
    ...
    if (HAL_FDCAN_AddMessageToTxFifoQ(this->hfdcan, &stCanTxMsg->TxHeader, 	stCanTxMsg->u8CanData) != HAL_OK)
    {
